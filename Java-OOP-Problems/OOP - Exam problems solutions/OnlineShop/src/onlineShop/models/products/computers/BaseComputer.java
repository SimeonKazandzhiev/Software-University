...
public abstract class BaseComputer extends BaseProduct implements Computer {
    ...
    public double getOverallPerformance() {
        double overallPerformanceComponentsAverage =
                components.stream()
                        .mapToDouble(Product::getOverallPerformance)
                        .average().orElse(0.00);
        return super.getOverallPerformance() + overallPerformanceComponentsAverage;
    ...
    public String toString() {
        ...
        for (Component component : components) {
            sb.append("  ").append(component.toString()).append(newLine);
        }
        sb.append(String.format(" " +COMPUTER_PERIPHERALS_TO_STRING,
                this.peripherals.size(),this.peripherals.stream()
                        .mapToDouble(Peripheral::getOverallPerformance).average().orElse(0.00)));
        for (Peripheral peripheral : peripherals) {
            sb.append("  ").append(peripheral).append(newLine);
        }
